...
import {
  createRepresentativeRemote,
  deleteUserRemote,
  getUserByIdRemote,
  listUsersRemote,
  updateOwnProfileRemote,
  updateUserRemote,
  isValidAuthPassword,
  resetRepresentativePasswordRemote,
} from './supabase/users';
...
export async function listUsers(): Promise<User[]> {
  if (isSupabaseConfigured()) return listUsersRemote();
...
export async function getUserById(id: string): Promise<User | null> {
  if (isSupabaseConfigured()) return getUserByIdRemote(id);
...
export async function createRepresentative(data: CreateRepresentativeData): Promise<User> {
  if (isSupabaseConfigured()) {
    return createRepresentativeRemote({
      name: data.name,
      email: data.email,
...
export async function updateUser(id: string, data: UpdateUserData): Promise<User> {
  if (isSupabaseConfigured()) {
    if (data.resetPinToDefault && data.email) {
      await resetRepresentativePasswordRemote(data.email);
    }
    return updateUserRemote(id, {
      name: data.name,
      email: data.email,
      categoryIds: data.categoryIds,
      photoUri: data.photoUri,
